<template>
  <article v-if="allReady" class="post">
    <section class="post__body rte">
      <project-header id="citytoll" theme="secondary">
        <template v-slot:header>My Project</template>
        Pollution is in the air
        <template v-slot:subheading>
          <div class="mt-4 mb-3">A smart city toll system (Cito) made second prizes.</div>
        </template>
        ...
        <template v-slot:article>
          ...
          <v-layout justify-center wrap>
            <base-text>
              It took place in the ministry itself that had been perfectly equipped with beds and all the stuff you need for a 24h hackathon. Even Minister Svenja Schulze herself showed up.
...
export default {
  ...
  computed: {
    allReady() {
      return this.ready && "cognac" === this.post;
    }
  },
  methods: {
    kebabify,
